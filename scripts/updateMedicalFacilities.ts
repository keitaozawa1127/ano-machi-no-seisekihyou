...
// 社会・人口統計体系（市区町村統計指標 - Ｉ 健康・医療）の統計表ID
const MEDICAL_STATS_ID = '0000020309';
const HOSP_CODE = '#I0910103'; // 一般病院数（人口10万人当たり）
const CLINIC_CODE = '#I0910105'; // 一般診療所数（人口10万人当たり）
const TARGET_TIME = '2020100000'; // 2020年度
...
// 医療機関データの取得
// ==========================================
async function fetchMedicalData(cityCode: string, cache: Record<string, number>): Promise<number | null> {
    ...
    // 病院数と診療所数を取得
    let total = 0;
    const codes = [HOSP_CODE, CLINIC_CODE];
    let foundAny = false;

    for (const code of codes) {
        const url = `${ESTAT_BASE_URL}/getStatsData?appId=${ESTAT_APP_ID}&statsDataId=${MEDICAL_STATS_ID}&cdCat01=${encodeURIComponent(code)}&cdArea=${cityCode}&cdTime=${TARGET_TIME}`;
        const json = await fetchEstatWithRetry(url);
        
        const valueStr = json?.GET_STATS_DATA?.STATISTICAL_DATA?.DATA_INF?.VALUE?.['$'];
        if (valueStr) {
            total += parseFloat(valueStr);
            foundAny = true;
        }
    }
    
    if (foundAny) {
        cache[cityCode] = total;
        return total;
    }
...
async function main() {
    ...
    console.log("=== 利便性：医療機関アクセス 自動取得・更新スクリプト開始 ===");
    ...
    // --- 1. インメモリキャッシュ ---
    const cacheMedical: Record<string, number> = {};
    ...
    for (const file of jsonFiles) {
        ...
        try {
            ...
            // --- 医療機関データの取得 ---
            const medicalValue = await fetchMedicalData(cityCode, cacheMedical);
            ...
            if (medicalValue === null || medicalValue < 0) {
                console.warn(`  > [Fallback] ${stationName}: データが取得できないため標準水準でフォールバックします。`);
                scoreImpact = 0;
                evaluationValue = "データなし（標準評価）";
                description = "標準的な数の医療機関が点在しており、日常の医療アクセスには困りません。";
                fallbackCount++;
            } else {
                evaluationValue = `${medicalValue.toFixed(1)}（人口10万人当たり）`;
                if (medicalValue >= 150) {
                    scoreImpact = 2;
                    description = "周辺に医療機関が充実しており、万が一の際や日常の通院・小児科等の利用に非常に安心な環境です。";
                } else if (medicalValue >= 80) {
                    scoreImpact = 0;
                    description = "標準的な数の医療機関が点在しており、日常の医療アクセスには困りません。";
                } else if (medicalValue >= 50) {
                    scoreImpact = -1;
                    description = "医療機関の数が比較的少なく、特定の診療科や夜間救急のアクセスについては事前確認が推奨されます。";
                } else {
                    scoreImpact = -2;
                    description = "医療機関の数が非常に限られています。隣接自治体の医療施設や、緊急時の搬送体制などを事前に確認しておく必要があります。";
                }
            }
            ...
            const existingIndex = data.ext.dynamicAdditions.findIndex((item: any) => item.label === "医療機関アクセス");
            if (existingIndex >= 0) {
