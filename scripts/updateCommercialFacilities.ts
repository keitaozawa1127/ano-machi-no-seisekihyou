...
// 社会・人口統計体系（市区町村統計指標 - Ｃ 経済（事業所数等））の統計表ID
const COMM_STATS_ID = '0000020203';
const RETAIL_CAT01_CODE = 'C350202'; // 小売業事業所数
const TARGET_TIME = '2021100000'; // 2021年度（経済センサス‐活動調査）
...
// 小売店舗データの取得
// ==========================================
async function fetchRetailData(cityCode: string, cache: Record<string, number>): Promise<number | null> {
    if (cache[cityCode] !== undefined) {
        return cache[cityCode];
    }

    const url = `${ESTAT_BASE_URL}/getStatsData?appId=${ESTAT_APP_ID}&statsDataId=${COMM_STATS_ID}&cdCat01=${encodeURIComponent(RETAIL_CAT01_CODE)}&cdArea=${cityCode}&cdTime=${TARGET_TIME}`;
...
async function main() {
    ...
    console.log("=== 利便性：買い物利便性（大型商業施設等） 自動取得・更新スクリプト開始 ===");
    ...
    // --- 1. インメモリキャッシュ ---
    const cacheRetail: Record<string, number> = {};
    ...
    for (const file of jsonFiles) {
        ...
        try {
            ...
            // --- 小売店舗データの取得 ---
            const retailCount = await fetchRetailData(cityCode, cacheRetail);
            ...
            if (retailCount === null || retailCount < 0) {
                console.warn(`  > [Fallback] ${stationName}: データが取得できないため標準水準でフォールバックします。`);
                scoreImpact = 0;
                evaluationValue = "データなし（標準評価）";
                description = "日常生活に必要な買い物環境が標準的に揃っています。";
                fallbackCount++;
            } else {
                evaluationValue = `${retailCount.toLocaleString()}店舗`;
                if (retailCount >= 1000) {
                    scoreImpact = 2;
                    description = "大型商業施設やスーパーが充実しており、日用品から休日のショッピングまで買い物の利便性が非常に高い環境です。";
                } else if (retailCount >= 300) {
                    scoreImpact = 0;
                    description = "日常生活に必要な買い物環境が標準的に揃っています。";
                } else if (retailCount >= 100) {
                    scoreImpact = -1;
                    description = "駅周辺の店舗数が限られており、日常の買い物にはやや工夫が必要な場合があります。";
                } else {
                    scoreImpact = -2;
                    description = "小売店舗が非常に少なく、日常の買い物には隣接自治体への移動や車が必要な可能性が高いエリアです。";
                }
            }
            ...
            const existingIndex = data.ext.dynamicAdditions.findIndex((item: any) => item.label === "買い物利便性（大型商業施設等）");
            if (existingIndex >= 0) {
