...
// 路線→都道府県の正しいマッピング辞書
const LINE_TO_PREFECTURE: { [key: string]: string } = {
    // 神奈川県 (14)
    '東急東横線': '14',
    'JR湘南新宿ライン': '14',
    'JR横須賀線': '14',
...
// 駅名→都道府県の直接マッピング（確実な駅）
const STATION_TO_PREFECTURE: { [key: string]: string } = {
    '武蔵小杉': '14',
    '横浜': '14',
    '新横浜': '14',
    '川崎': '14',
...
async function repairStationData() {
    ...
    Object.entries(rawData).forEach(([key, station]: [string, any]) => {
    ...
    testStations.forEach(name => {
        const station = Object.values(rawData).find((s: any) => s.name === name);
        if (station) {
            const prefCode = String((station as any).prefCode).padStart(2, '0');
            const prefName = PREFECTURES.find(p => p.code === prefCode)?.name;
            const expectedCode = expected[name as keyof typeof expected];
