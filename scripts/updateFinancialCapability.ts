...
// 社会・人口統計体系（市区町村統計指標 - Ｄ 行財政）の統計表ID
const FINANCIAL_STATS_ID = '0000020204';
const FINANCIAL_CAT01_CODE = 'D2201'; // 財政力指数（市町村財政）
const TARGET_TIME = '2021100000'; // 2021年度 (最新利用可能データ)
...
// 財政力指数データの取得
// ==========================================
async function fetchFinancialData(cityCode: string, cache: Record<string, number>): Promise<number | null> {
    if (cache[cityCode] !== undefined) {
        return cache[cityCode];
    }

    const url = `${ESTAT_BASE_URL}/getStatsData?appId=${ESTAT_APP_ID}&statsDataId=${FINANCIAL_STATS_ID}&cdCat01=${FINANCIAL_CAT01_CODE}&cdArea=${cityCode}&cdTime=${TARGET_TIME}`;
    ...
    // 特別区などの場合、データが欠損している可能性があるため、親のコード（13000等）を試すロジックは今回含まず
    // cityCodeが3区（特別区）の場合は、東京都のデータが出るわけではないので、個別に取得できない場合はnull
    cache[cityCode] = -1; // 欠損
    return null;
}
...
async function main() {
    ...
    console.log("=== 将来性：財政力指数（自治体の財政余力） 自動取得・更新スクリプト開始 ===");
    ...
    // --- 1. インメモリキャッシュ ---
    const cacheFinancial: Record<string, number> = {};
    ...
    for (const file of jsonFiles) {
        ...
        try {
            ...
            // --- 財政力指数データの取得 ---
            const financialValue = await fetchFinancialData(cityCode, cacheFinancial);
            ...
            if (financialValue === null || financialValue < 0) {
                console.warn(`  > [Fallback] ${stationName}: データが取得できないため標準水準でフォールバックします。`);
                scoreImpact = 0;
                evaluationValue = "データなし（標準評価）";
                description = "全国的に標準的な財政水準です。";
                fallbackCount++;
            } else {
                evaluationValue = financialValue.toFixed(2);
                if (financialValue >= 1.0) {
                    scoreImpact = 2;
                    description = "財政力指数が1.0を超えており、極めて健全な財政基盤を持ちます。将来的なインフラ維持や行政サービスの拡充に大きな期待が持てます。";
                } else if (financialValue >= 0.7) {
                    scoreImpact = 1;
                    description = "良好な財政状態を維持しており、安定した行政サービスが期待できます。";
                } else if (financialValue >= 0.4) {
                    scoreImpact = 0;
                    description = "全国的に標準的な財政水準です。";
                } else if (financialValue >= 0.2) {
                    scoreImpact = -1;
                    description = "財政力指数が低めであり、将来的な行政サービスやインフラ維持の動向に注視が必要です。";
                } else {
                    scoreImpact = -2;
                    description = "財政力指数が非常に低く、自治体の財政基盤に課題があります。将来的なインフラ更新やサービス維持のリスクを考慮する必要があります。";
                }
            }
            ...
            const existingIndex = data.ext.dynamicAdditions.findIndex((item: any) => item.label === "財政力指数（自治体の財政余力）");
            if (existingIndex >= 0) {
